...
public class RelicDictionary : FrameworkDictionary
{
    ...
    private void AddToList(int ID, string name, string disc, Classes cardClass, int[] shopCostRange, List<FuncArgs> effects)
    {
    ...
    { //ID,Name,ManaCost,Description,Image,Class,EffectList
        ...
        AddToList(0, "Spinach", "Gain 1 strength for each 2 rolled in your last roll", Classes.Warrior,new int[] {27,41}, new List<FuncArgs>() { new FuncArgs(cardGameManager.ApplyStatus, cardGameManager.EffectOnSelf,boardManager.GetCurrentDiceRoll,2,0, EffectTiming.EnterCombat,Status.strength) });
    }
}
